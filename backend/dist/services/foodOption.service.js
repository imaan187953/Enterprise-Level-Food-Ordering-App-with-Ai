...
export const createFoodOption = async (data) => {
    ...
    if (!food.isActive) {
        throw new ApiError(400, "Cannot add options to an inactive food");
    }
    ...
    if (existingOption) {
        throw new ApiError(409, "This food option already exists");
    }
...
export const getFoodOptions = async (foodId) => {
    const filter = {
        isActive: true,
    };
    if (foodId) {
...
export const getFoodOptionById = async (id) => {
    if (!mongoose.Types.ObjectId.isValid(id)) {
        throw new ApiError(400, "Invalid food option ID");
    }
    const option = await FoodOption.findById(id).populate("food", "name slug price");
    if (!option) {
        throw new ApiError(404, "Food option not found");
    }
    return option;
};
export const getFoodOptionsByGroup = async (foodId, groupName) => {
    if (!mongoose.Types.ObjectId.isValid(foodId)) {
        throw new ApiError(400, "Invalid food ID");
    }
...
export const updateFoodOptionStatus = async (id, data) => {
    if (!mongoose.Types.ObjectId.isValid(id)) {
        throw new ApiError(400, "Invalid food option ID");
    }
    const option = await FoodOption.findById(id);
...
//# sourceMappingURL=foodOption.service.js.map
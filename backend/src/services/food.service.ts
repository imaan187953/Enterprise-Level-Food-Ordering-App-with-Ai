...
export const createFood = async (
  ...
  if (existingFood) {
    throw new ApiError(
      409,
      "A food item with this name or slug already exists"
    );
  }
  ...
  if (!categoryExists.isActive) {
    throw new ApiError(
      400,
      "Cannot add food to an inactive category"
    );
  }
...
export const updateFood = async (
  ...
  food.category = new mongoose.Types.ObjectId(
    data.category
  );
}
